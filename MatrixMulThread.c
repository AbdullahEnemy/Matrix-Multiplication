...
void *multiplyMatrices(void *args) {
	...
	for (int i = thread_args->start_row; i < thread_args->end_row; ++i) {
    	for (int j = 0; j < thread_args->cols2; ++j) {
        	thread_args->result[i][j] = 0;
        	for (int k = 0; k < thread_args->cols1; ++k) {
            	thread_args->result[i][j] += thread_args->matrix1[i][k] * thread_args->matrix2[k][j];
        	}
    	}
	}
	...
	if (opcode == 5) {
    	if (matrixType == 1) {
        	...
        	for (int i = 0; i < MAX_THREADS; ++i) {
            	...
            	pthread_create(&threads[i], NULL, multiplyMatrices, (void *)&thread_args[i]);
        	}
